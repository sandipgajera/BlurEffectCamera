...
public class CropActivity extends Activity {
    ...
    private class CropTask extends MyAsyncTask<Void, Void, Void> {
        ...
        protected void onPreExecute() {
            if (CropActivity.this.outputPath.contains("jpg")
                    || CropActivity.this.outputPath.contains("jpeg")
                    || CropActivity.this.outputPath.contains("png")) {
                this.localOutputPath = CropActivity.this.outputPath;
            } else {
                this.localOutputPath = CropActivity.this.outputPath.substring(
                        0, CropActivity.this.outputPath
                                .lastIndexOf(File.separator) + 1);
                this.localOutputPath += "crop.jpg";
            }
            try {
                this.progressDialog = new ProgressDialog(
                        CropActivity.this.context);
                this.progressDialog.setMessage(CropActivity.this
                        .getString(R.string.crop_getting_crop_image));
        ...
        protected Void doInBackground(Void... arg0) {
            if (!CropActivity.this.isHDR) {
                this.res = CropActivity.crop(CropActivity.this.inputPath,
                        this.localOutputPath,
                        CropActivity.this.cropView.getLeftPos(),
                        CropActivity.this.cropView.getTopPos(),
                        CropActivity.this.cropView.getRightPos(),
                        CropActivity.this.cropView.getBottomPos(),
                        CropActivity.this.cropView.rotation);
            }
            return null;
        }
        ...
        protected void onPostExecute(Void unused) {
            if (this.localOutputPath != CropActivity.this.outputPath) {
                new File(this.localOutputPath).renameTo(new File(
                        CropActivity.this.outputPath));
            }
            try {
            ...
            if (CropActivity.this.isHDR) {
                ...
                plasmaIntent.putExtra("result_path",
                        CropActivity.this.outputPath);
                plasmaIntent.putExtra("bundle", CropActivity.this.bundle);
                plasmaIntent.putExtra("left",
                        CropActivity.this.cropView.getLeftPos());
                plasmaIntent.putExtra("top",
                        CropActivity.this.cropView.getTopPos());
                plasmaIntent.putExtra("right",
                        CropActivity.this.cropView.getRightPos());
                plasmaIntent.putExtra("bottom",
                        CropActivity.this.cropView.getBottomPos());
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        this.cropView = new CropView(this, this.inputPath, screenWidth,
                screenHeight, null, 1);
        if (this.cropView.bitmap == null) {
            Toast.makeText(getApplicationContext(),
                    R.string.crop_image_load_error_message, Toast.LENGTH_LONG)
                    .show();
            finish();
            return;
        }
    ...
    void setRatioButtonListBackgroundColor(int index) {
        for (int i = 0; i < this.ratioButtonList.length; i++) {
            this.ratioButtonList[i]
                    .setBackgroundResource(R.drawable.crop_border);
            this.ratioButtonList[i]
                    .setTextColor(ViewCompat.MEASURED_STATE_MASK);
        }
        this.ratioButtonList[index]
                .setBackgroundResource(R.drawable.crop_border_selected);
